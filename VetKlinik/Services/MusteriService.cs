...
namespace VetKlinik.Services
{
    public class MusteriService : IMusteriService
    {
        ...
        {
            var gelenMusteri = _ApplicationDbContext.Musteriler.Where(x => x.Id==id).FirstOrDefault();
        ...
        {
            return _ApplicationDbContext.Musteriler.Where(x => x.Id == id).FirstOrDefault();
        }
